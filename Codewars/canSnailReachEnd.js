...
The snail moves at a constant speed of y units per minute. As the snail crawls
 from the left end of the rubber band to the right end, the rubber band increases
  in length from the right side every minute, adding z units to its length.

The question is: Will the snail be able to reach the right end of the rubber band within 1 year? (= 365 days)
...
Mathematics
*/
const canSnailReachEndMREAproach = (length, speed, lengthIncreases) => ((speed / lengthIncreases) * Math.log((length + lengthIncreases * (365 * 24 * 60)) / length) >= 1);


const canSnailReachEnd = (length, speed, lengthIncreases) => {
      if(speed <= lengthIncreases) return false;
      return length / (speed - lengthIncreases) <=(365 * 24 * 60)
};