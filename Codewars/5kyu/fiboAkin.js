...
Task:
    ...
    or lengthSupUK or length-sup-u-k returning the number of terms u[i] >= k with
    1 <= i <= n. If we look above we can see that between u[1] and u[23] we have
    four u[i] greater or equal to 12: length_sup_u_k(23, 12) => 4

Given n (integer > 2) write the function comp(n) (cmp in COBOL) returning
the number of times where a term of u is less than its predecessor up to and including u[n].
...
// u[n] = u[n − u[n − 1]] + u[n − u[n]

/////////TEST CASES////////

console.log(lengthSupUK(23, 12)); // 4
console.log(lengthSupUK(50, 10)); // 35
console.log(lengthSupUK(500, 100)); // 304

console.log(comp(23));  // 1
console.log(comp(100)); // 22
console.log(comp(200)); // 63